...
| 分发 | PyPI 软件包 `1point3acres-toolkit`，用 hatchling 打包；`server.json` 登记到 MCP 官方目录 | 不想克隆仓库的人一条命令就能装；目录里的 AI 客户端能直接找到它 |
| 本机 OCR | rapidocr-onnxruntime 1.4.4 | 面经里的截图在本机识别文字，不上传 |
| 时区 | tzdata 2026.3 | Windows 没有系统时区数据库，`zoneinfo` 需要它 |
| 测试 | Python 标准库 `unittest`，36 个测试文件、425 个用例 | 不引入测试框架；跳过也算失败 |
| CI | GitHub Actions：Ubuntu 静态检查（顺带构建一次软件包并核对内容）→ Windows 2022 / 2025 + macOS 15 单元测试 → Windows 2025 + macOS 15 集成测试 → 汇总 | 和本地检查命令跑的是同一个 `check.py` |
...
| 模块 | 职责 | 行数 |
|:--|:--|--:|
| `cli.py` | 命令行入口：解析子命令，打印 JSON，决定退出码 | 259 |
| `mcp_server.py` | MCP 服务：把同样的函数包成 32 个工具，多余参数直接拒绝 | 277 |
| `entry.py` | 安装版的两个命令入口：先把包目录放回脚本布局，再交给 `mcp_server` 或 `cli` | 27 |
| `check.py` | 唯一的离线检查入口：阶段选择、依赖校验、跑测试、生成报告 | 120 |
| `governance.py` | 用 AST 扫源码：依赖白名单、外部库归属、不许写状态字面量、生成文件是否过期、文档链接是否断、打包文件是否一致 | 306 |
...
| `settings.py` | 所有配置、默认值、路径、限额；不含任何密码；两种安装方式的目录约定 | 250 |

...
- **计划时间** 默认 16:10（北京时间）。在账号配置里可以改时间和时区，不要改仓库里的代码，那会让你的副本和主分支分叉。
- **叫醒方式** 推荐每 10 分钟叫醒一次。它自己判断该不该跑，没到点、已完成时直接退出、不开浏览器。
- **恢复时点** 是给「只能按固定时间点触发」的定时任务准备的：从计划时间起每 4 小时一个。检查命令会把本地时区和 UTC 两套时刻都打印出来，照着填就行。

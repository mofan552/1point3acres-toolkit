...
签到页的「说说」会成为主页上的公开记录。默认随机选择心情和短句；这些是模板表达，不是根据个人生活生成的日记。
...
- 词库保留 109 句不描述具体个人经历的短句。去重覆盖之前 30 个日历日；候选用尽或抽到「没心情」时，使用网站支持的「没心情」空文案签到，不重复旧句。
...
- **失败恢复** 按已存记录等待 5、10、20、40、60 分钟，上限 60 分钟；`recovery_wait` 期间不访问网站。等待是调度状态，不能当作当天业务成功。
